 ...
 *
 * Dla potrzeb obsługi Chaos Monkey, adnotujemy ten komponent {@link Repository}, gdyż wtedy
 * Chaos Monkey będzie w stanie stworzyć proxy wokół tego komponentu i zmienić jego zachowanie.
 */
@Repository
class JdbcVerificationRepository implements VerificationRepository {
	...
	public VerifiedPerson save(VerifiedPerson entity) {
		...
		jdbcTemplate.update("INSERT INTO verified " +
						...
						"status) VALUES " +
						"(nextval('verified_id_seq'), ?, ?, ?)",
				entity.getUserId(), entity.getNationalIdentificationNumber(), entity.getStatus());
		return entity;
	}
