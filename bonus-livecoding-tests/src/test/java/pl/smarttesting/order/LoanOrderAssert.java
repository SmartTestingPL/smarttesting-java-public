...
class LoanOrderAssert {
	...
	LoanOrderAssert hasPromotion(String promotionName) {
		assertThat(loanOrder.getPromotions())
				.filteredOn(promotion -> promotion.name().equals(promotionName))
				.size().isEqualTo(1);
		return this;
	}
	...
	LoanOrderAssert firstPromotionHasDiscountValue(BigDecimal number) {
		assertThat(loanOrder.getPromotions().get(0).discount()).isEqualTo(number);
		return this;
	}

	LoanOrderAssert correctStudentLoan() {
